...
import { Search, Plus, Pencil, Trash2, Layers, X } from 'lucide-react'
...
export default function CompositesManager({ initial, costs, canEdit }: { initial: Composite[]; costs: Record<string, number>; canEdit: boolean }) {
  ...
  async function remove(id: string) { if (confirm('Eliminar este artigo?')) { await supabase.from('composites').delete().eq('id', id); refresh() } }
    ...
    <div>
      ...
      <div className="bg-[var(--surface)] border border-[var(--border)] rounded-[4px] overflow-hidden">
        <table className="w-full">
          <thead className="bg-[rgba(26,26,26,0.02)] border-b border-[var(--border)]"><tr><th className={th}>Artigo</th><th className={th}>Categoria</th><th className={th}>Unidade</th><th className={`${th} text-right`}>Custo real</th>{canEdit && <th className={`${th} text-right`}>Ações</th>}</tr></thead>
          <tbody className="divide-y divide-[var(--border)]">
            {visible.length === 0 && <tr><td colSpan={canEdit ? 5 : 4} className="px-4 py-8 text-center text-sm text-[var(--muted)]">Sem artigos.</td></tr>}
            {visible.map(row => (
              <tr key={row.id} onClick={() => router.push(`/materiais/c/${row.id}`)} className="hover:bg-[rgba(26,26,26,0.02)] cursor-pointer">
                <td className={td}><div className="flex items-center gap-3"><Layers size={15} className="text-[var(--muted)]" /><span className="font-medium">{row.name}</span></div></td>
