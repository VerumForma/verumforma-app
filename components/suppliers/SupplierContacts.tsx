...
import type { SupplierContact } from '@/lib/supabase/types'
...
type Draft = Partial<SupplierContact>

export default function SupplierContacts({ supplierId, initial, canEdit, orgName }: { supplierId: string; initial: SupplierContact[]; canEdit: boolean; orgName?: string }) {
  const supabase = createClient()
  const [rows, setRows] = useState<SupplierContact[]>(initial)
  const [draft, setDraft] = useState<Draft | null>(null)
  const [saving, setSaving] = useState(false)
  const [error, setError] = useState('')

  async function refresh() {
    const { data } = await supabase.from('supplier_contacts').select('*').eq('supplier_id', supplierId).order('is_primary', { ascending: false }).order('name')
    setRows((data ?? []) as SupplierContact[])
  }
  async function save() {
    ...
    const { error } = draft.id
      ? await supabase.from('supplier_contacts').update(base).eq('id', draft.id)
      : await supabase.from('supplier_contacts').insert({ ...base, supplier_id: supplierId })
    setSaving(false)
    if (error) { setError(error.message); return }
    setDraft(null); refresh()
  }
  async function remove(id: string) { if (confirm('Eliminar este contacto?')) { await supabase.from('supplier_contacts').delete().eq('id', id); refresh() } }
