...
export default function SuppliersManager({ initial, canEdit }: { initial: Supplier[]; canEdit: boolean }) {
  ...
  async function save() {
    ...
    const payload = {
      ...
      labour_type: (draft.supplies ?? []).includes('mao_obra') ? (draft.labour_type || null) : null,
        ...
        <div className="fixed inset-0 z-40 bg-black/40 flex items-start justify-center overflow-y-auto p-4">
          <div className="bg-white rounded-[6px] w-full max-w-2xl my-8 p-6 md:p-8">
            <div className="flex items-center justify-between mb-1">
              <h2 className="font-playfair text-2xl">{draft.id ? 'Editar' : 'Adicionar'} fornecedor</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
              ...
              <div>
                ...
                <select className={input} value={draft.kind ?? 'individual'} onChange={e => setDraft({ ...draft, kind: e.target.value as Supplier['kind'] })}>
                  <option value="individual">Individual</option>
                  <option value="empresa">Empresa</option>
                </select>
              </div>
              <div>
                ...
                <select className={input} value={draft.status ?? 'potencial'} onChange={e => setDraft({ ...draft, status: e.target.value as Supplier['status'] })}>
                  <option value="potencial">Potencial</option>
                  <option value="ativo">Ativo</option>
                  <option value="inativo">Inativo</option>
                </select>
              ...
              <div className="md:col-span-2">
                <label className={label}>Fornece</label>
                <div className="flex flex-wrap gap-4">
                  {SUPPLIES.map(sp => (
                    <label key={sp.key} className="inline-flex items-center gap-2 text-sm">
                      <input type="checkbox" checked={draft.supplies?.includes(sp.key) ?? false} onChange={e => { const cur = draft.supplies ?? []; setDraft({ ...draft, supplies: e.target.checked ? [...cur, sp.key] : cur.filter(k => k !== sp.key) }) }} />
                      {sp.label}
                    </label>
                  ))}
                </div>
              </div>
              {(draft.supplies ?? []).includes('mao_obra') && (
                <div className="md:col-span-2">
                  <label className={label}>Tipo de mão de obra</label>
                  <input className={input} value={draft.labour_type ?? ''} onChange={e => setDraft({ ...draft, labour_type: e.target.value })} />
                </div>
              )}
            </div>
