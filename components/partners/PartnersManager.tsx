      ...
      <div className="bg-[var(--surface)] border border-[var(--border)] rounded-[4px] overflow-hidden">
        <table className="w-full">
          ...
          <tbody className="divide-y divide-[var(--border)]">
            {visible.length === 0 && (
              <tr><td colSpan={canEdit ? 6 : 5} className="px-4 py-8 text-center text-sm text-[var(--muted)]">Sem parceiros{q || fEstado !== 'all' || fTipo !== 'all' ? ' para este filtro' : ' ainda'}.</td></tr>
            )}
            {visible.map(row => (
              <tr key={row.id} onClick={() => router.push(`/parceiros/${row.id}`)} className="hover:bg-[rgba(26,26,26,0.02)] cursor-pointer">
                <td className={td}>
