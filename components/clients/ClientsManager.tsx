...
export default function ClientsManager({ initial, canEdit }: { initial: Client[]; canEdit: boolean }) {
  ...
  const accessors = { name: (r: Client) => r.name, email: (r: Client) => r.email, phone: (r: Client) => r.phone, kind: (r: Client) => r.kind, status: (r: Client) => r.status }
  const { sorted, sortKey, sortDir, toggle } = useTableSort(visible, accessors, 'name', 'asc', 'clientes')
  ...
  async function bulkDelete() { const ids = sel.selectedIds(); if (!ids.length) return; await supabase.from('clients').delete().in('id', ids); sel.clear(); refresh() }
  ...
  return (
    <div className="w-full">
      ...
      {canEdit && <SelectionBar count={sel.count} onClear={sel.clear} onDelete={bulkDelete} noun="clientes" />}
              ...
              <tr key={row.id} onClick={() => router.push(`/clientes/${row.id}`)} className="hover:bg-[rgba(26,26,26,0.02)] cursor-pointer">{canEdit && <td className={td} onClick={e => e.stopPropagation()}><input type="checkbox" checked={sel.isSelected(row.id)} onChange={e => sel.toggle(row.id, (e.nativeEvent as MouseEvent).shiftKey)} className="accent-[#1A1A1A] cursor-pointer align-middle" aria-label="Selecionar" /></td>}
                ...
                {canEdit && (
                  <td className={`${td} text-right whitespace-nowrap`}>
                    ...
                    <ConfirmButton stop onConfirm={() => remove(row.id)} message="Eliminar este cliente?" className="text-[var(--muted)] hover:text-red-500 p-1 ml-1" title="Eliminar"><Trash2 size={15} /></ConfirmButton>
                  </td>
                )}
              </tr>
            ))}
