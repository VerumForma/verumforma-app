...
export default async function MaterialDetailPage({ params }: { params: { id: string } }) {
  ...
  if (!data) notFound()
  const [{ data: prices }, { data: suppliers }, { data: units }] = await Promise.all([
  ...
  return <MaterialDetail material={data as Material} prices={(prices ?? []) as MaterialPrice[]} suppliers={(suppliers ?? []) as { id: string; name: string }[]} units={(units ?? []) as MaterialUnit[]} canEdit={canEdit(level)} />
}
